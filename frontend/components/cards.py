...
def village_grid_card(item, on_map_click=None):
    with ui.card().classes(
        ...
        # Content
        with ui.column().classes('p-5 gap-3 flex-1'):
            with ui.column().classes('gap-1'):
                ui.label(item.get('name', 'Làng Quan họ')).classes('font-display text-xl font-bold text-foreground group-hover:text-primary transition-colors cursor-pointer').on('click', lambda: ui.navigate.to(f'/lang-quan-ho/{item.get("id")}'))
                with ui.row().classes('items-center gap-2 text-sm text-muted-foreground'):
                    ui.icon('place', size='16px').classes('text-primary')
            ...
            # Stats & Info
            with ui.row().classes('w-full justify-between items-center bg-muted/30 p-3 rounded-xl border border-border/50'):
                with ui.column().classes('items-center'):
                    ui.label(str(item.get('artist_count', 0))).classes('text-lg font-bold text-primary')
                    ui.label(t('artists')).classes('text-[10px] uppercase font-bold text-muted-foreground')
                
                ui.element('div').classes('w-[1px] h-8 bg-border')
                
                with ui.column().classes('flex-1 px-4'):
                    ui.label('Làn điệu đặc trưng').classes('text-[10px] uppercase font-bold text-muted-foreground mb-1')
                    songs = item.get('featured_songs', 'Đang cập nhật')
                    ui.label(songs).classes('text-[11px] font-medium text-foreground line-clamp-1')

            # Action
            with ui.row().classes('w-full mt-auto pt-2 gap-3'):
                ui.button(t('card_view_detail'), on_click=lambda: ui.navigate.to(f'/lang-quan-ho/{item.get("id")}')).props('outline color="primary" rounded').classes('flex-1 text-xs font-bold')
                if on_map_click:
                    ui.button(t('card_map'), icon='map', on_click=on_map_click).props('unelevated color="primary" rounded').classes('flex-1 text-xs font-bold')

def news_grid_card(item):
    with ui.card().classes('w-full p-0 flex flex-col overflow-hidden hover:shadow-elevated transition-all duration-300 cursor-pointer bg-card border border-border group h-full').on('click', lambda: ui.navigate.to(f'/tin-tuc/{item.get("id")}')):
        ...
        with ui.column().classes('p-6 flex-grow w-full gap-0'):
            with ui.row().classes('justify-between items-center w-full mb-3'):
                ui.label(item.get('category', 'Tin tức')).classes('text-[10px] font-bold text-primary uppercase tracking-wider bg-primary/10 px-2.5 py-1 rounded-sm')
                date_str = (item.get('created_at') or '--/--/----')[:10]
                with ui.row().classes('items-center gap-1 text-muted-foreground'):
                    ui.icon('calendar_today', size='12px')
                    ui.label(date_str).classes('text-xs font-medium')
                    
            ui.label(item.get('title', 'Không có tiêu đề')).classes('text-lg font-bold font-display line-clamp-2 mb-2 group-hover:text-primary transition-colors leading-snug')
            ui.label(item.get('description', 'Mô tả ngắn gọn về tin tức này sẽ được hiển thị ở đây để người dùng dễ dàng theo dõi và nắm bắt thông tin...')).classes('text-sm text-muted-foreground line-clamp-3 mb-4 flex-grow')
...
def event_grid_card(item, on_register=None):
    with ui.card().classes('w-full p-0 flex flex-col overflow-hidden hover:shadow-elevated transition-all duration-300 bg-card border border-border group h-full'):
        ...
        with ui.column().classes('p-6 flex-grow w-full gap-0 relative'):
            ui.label(item.get('title', 'Không có tiêu đề')).classes('text-lg font-bold font-display line-clamp-2 mb-2 hover:text-primary transition-colors cursor-pointer leading-snug').on('click', lambda: ui.navigate.to(f'/tin-tuc/{item.get("id")}'))
            ...
            ui.label(item.get('description', '')).classes('text-sm text-muted-foreground line-clamp-2 mb-4')
            
            with ui.column().classes('w-full gap-2.5 mb-6 bg-muted/30 p-4 rounded-xl border border-border/50'):
                with ui.row().classes('items-start gap-2.5 text-sm text-foreground flex-nowrap'):
                    ui.icon('place', size='18px').classes('text-primary mt-0.5 shrink-0')
                    ui.label(item.get('location', 'Tại địa phương')).classes('line-clamp-2 font-medium leading-tight')
                with ui.row().classes('items-center gap-2.5 text-sm text-foreground'):
                    ui.icon('group', size='18px').classes('text-primary shrink-0')
                    slots = item.get('available_slots', 0)
                    ui.label(f"Số lượng: Còn {slots} chỗ" if slots > 0 else "Số lượng: Đã hết chỗ").classes(f'font-medium {"text-negative" if slots <= 0 else ""}')
