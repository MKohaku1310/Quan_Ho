...
async def admin_villages_page():
    ...
    async def do_delete():
        ...
        if await api_client.delete_location(state.delete_id):
            ...
            with theme.container():
                with ui.row().classes('w-full justify-between items-center mb-12'):
                    with ui.column().classes('gap-1'):
                        with ui.link(target='/admin').classes('flex items-center gap-1 text-primary no-underline hover:underline mb-2'):
                            ...
                            ui.label(t('back_btn'))
                        ui.label(t('manage_villages')).classes('text-4xl font-display font-bold')
                    
                    ui.button(t('add_prefix') + ' ' + t('et_village'), icon='add', 
                              on_click=lambda: ui.navigate.to('/admin/edit/village/0')).props('unelevated rounded-xl color=primary').classes('px-6 shadow-lg shadow-primary/20')
                ...
                with ui.card().classes('w-full p-8 rounded-[2rem] shadow-xl border border-border bg-card'):
                    ...
                    async def content_area():
                        ...
                        state.items = await api_client.get_locations(skip=skip, limit=state.items_per_page)
                        ...
                        if not state.items:
                            ...
                            with ui.row().classes('w-full bg-muted/20 p-4 font-black text-[10px] tracking-widest text-muted-foreground uppercase rounded-t-xl'):
                                ui.label(t('et_village')).classes('flex-1')
                                ui.label(t('field_district')).classes('w-32 text-center')
                            ...
                            for item in state.items:
                                with ui.row().classes('w-full items-center p-4 border-b border-border/50 hover:bg-muted/30 transition-colors'):
                                    ...
                                    with ui.element('div').classes('w-32 flex justify-center'):
                                        ui.label(item.get('district', t('updating'))).classes('px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-secondary/10 text-secondary')

                                    with ui.row().classes('w-32 justify-end gap-2'):
                                        ui.button(icon='edit', on_click=lambda i=item: ui.navigate.to(f'/admin/edit/village/{i["id"]}')).props('flat round size=sm color=primary')
