...
async def admin_news_page():
    ...
    async def do_delete():
        ...
        if await api_client.delete_article(state.delete_id):
                ...
                with ui.row().classes('w-full justify-between items-center mb-12'):
                    with ui.column().classes('gap-1'):
                        with ui.link(target='/admin').classes('flex items-center gap-1 text-primary no-underline hover:underline mb-2'):
                            ...
                            ui.label(t('back_btn'))
                        ui.label(t('manage_news')).classes('text-4xl font-display font-bold')
                    
                    ui.button(t('add_prefix') + ' ' + t('et_news'), icon='add', 
                              on_click=lambda: ui.navigate.to('/admin/edit/news/0')).props('unelevated rounded-xl color=primary').classes('px-6 shadow-lg shadow-primary/20')

                with ui.card().classes('w-full p-8 rounded-[2rem] shadow-xl border border-border bg-card'):
                    @ui.refreshable
                    async def content_area():
                        state.total_count = await api_client.get_articles_count()
                        skip = (state.page - 1) * state.items_per_page
                        state.items = await api_client.get_articles(skip=skip, limit=state.items_per_page)
                        ...
                        if not state.items:
                            ...
                            with ui.row().classes('w-full bg-muted/20 p-4 font-black text-[10px] tracking-widest text-muted-foreground uppercase rounded-t-xl'):
                                ui.label(t('et_news')).classes('flex-1')
                            ...
                            for item in state.items:
                                with ui.row().classes('w-full items-center p-4 border-b border-border/50 hover:bg-muted/30 transition-colors'):
                                    with ui.row().classes('flex-1 items-center gap-4'):
                                        ...
                                        ui.label(tc(item, 'title')).classes('font-bold')
                                    
                                    cat = item.get('category', 'tin-tuc')
                                    cat_label = { 'tin-tuc': t('cat_news'), 'le-hoi': t('cat_festival'), 'nghe-thuat': t('cat_art') }.get(cat, cat)
                                    colors = {'tin-tuc': 'bg-blue-100 text-blue-700', 'le-hoi': 'bg-primary/10 text-primary', 'nghe-thuat': 'bg-jade/10 text-jade'}
                                    ...
                                    with ui.row().classes('w-32 justify-end gap-2'):
                                        ui.button(icon='edit', on_click=lambda i=item: ui.navigate.to(f'/admin/edit/news/{i["id"]}')).props('flat round size=sm color=primary')
