...
async def admin_artists_page():
    ...
    async def do_delete():
        ...
        if await api_client.delete_artist(state.delete_id):
            ...
            with theme.container():
                with ui.row().classes('w-full justify-between items-center mb-12'):
                    with ui.column().classes('gap-1'):
                        with ui.link(target='/admin').classes('flex items-center gap-1 text-primary no-underline hover:underline mb-2'):
                            ...
                            ui.label(t('back_btn'))
                        ui.label(t('manage_artists')).classes('text-4xl font-display font-bold')
                    
                    ui.button(t('add_prefix') + ' ' + t('et_artist'), icon='add', 
                              on_click=lambda: ui.navigate.to('/admin/edit/artist/0')).props('unelevated rounded-xl color=primary').classes('px-6 shadow-lg shadow-primary/20')
                ...
                with ui.card().classes('w-full p-8 rounded-[2rem] shadow-xl border border-border bg-card'):
                    ...
                    async def content_area():
                        ...
                        state.items = await api_client.get_artists(skip=skip, limit=state.items_per_page)
                        ...
                        if not state.items:
                            ...
                            with ui.row().classes('w-full bg-muted/20 p-4 font-black text-[10px] tracking-widest text-muted-foreground uppercase rounded-t-xl'):
                                ui.label(t('et_artist')).classes('flex-1')
                                ui.label(t('field_generation')).classes('w-32 text-center')
                            ...
                            for item in state.items:
                                with ui.row().classes('w-full items-center p-4 border-b border-border/50 hover:bg-muted/30 transition-colors'):
                                    ...
                                    gen_label = t('gen_traditional') if gen == 'truyen-thong' else t('gen_new')
                                    bg_color = 'bg-primary/10 text-primary' if gen == 'truyen-thong' else 'bg-secondary/10 text-secondary'
                                    with ui.element('div').classes('w-32 flex justify-center'):
                                        ui.label(gen_label).classes(f'px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider {bg_color}')

                                    with ui.row().classes('w-32 justify-end gap-2'):
                                        ui.button(icon='edit', on_click=lambda i=item: ui.navigate.to(f'/admin/edit/artist/{i["id"]}')).props('flat round size=sm color=primary')
