...
def introduction_page():
    with theme.frame():
        ...
        with ui.element('section').classes('relative min-h-[50vh] flex items-center justify-center overflow-hidden w-full').style('padding-top: 56px;'):
            ...
            with ui.column().classes('relative z-10 text-center items-center px-4 gap-4'):
                ui.label('LỊCH SỬ VÀ GIÁ TRỊ VĂN HÓA').classes('text-[10px] font-bold tracking-[0.4em] text-gold-light uppercase animate-fade-in')
        ...
        with ui.element('section').classes('py-16 bg-background w-full relative overflow-hidden'):
             # Decorative pattern
            ui.image('/static/lotus-ornament.png').classes('absolute -left-20 -top-20 w-64 h-64 opacity-[0.03] rotate-12 pointer-events-none')
            ...
            with theme.container():
                ...
                with ui.column().classes('max-w-4xl mx-auto text-center items-center gap-6 mb-16'):
                    ui.label('Dân ca Quan họ Bắc Ninh là một hình thức hát giao duyên đối đáp giữa nam (liền anh) và nữ (liền chị), phổ biến tại vùng Kinh Bắc xưa.').classes('text-muted-foreground text-lg leading-relaxed font-light')
                    ui.label('Năm 2009, Quan họ được UNESCO vinh danh là Di sản Văn hóa Phi vật thể đại diện của Nhân loại, khẳng định sức sống mãnh liệt của di sản.').classes('bg-primary/5 px-6 py-4 rounded-2xl border border-primary/10 text-foreground italic text-sm md:text-base')
        ...
        # Compact Costume Section
        with ui.element('section').classes('py-20 bg-background/50 border-y border-border w-full relative overflow-hidden'):
            # Subtle background text
            ui.label('TRUYỀN THỐNG').classes('absolute -right-20 top-20 text-[12vw] font-black text-primary/5 select-none pointer-events-none uppercase')
